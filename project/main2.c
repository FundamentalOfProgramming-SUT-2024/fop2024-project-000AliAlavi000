...
void generate_map()
{
    // مقداردهی اولیه نقشه با دیوارهای اطراف
    for (int y = 0; y < HEIGHT; y++)
    {
        for (int x = 0; x < WIDTH; x++)
        {
            ...
            {
                map[y][x] = WALL; // دیوارهای اطراف
            }
            else
            {
                // المان‌های داخلی نقشه به‌صورت تصادفی
                int rand_val = rand() % 100;
                if (rand_val < 60)
                    map[y][x] = FLOOR; // 60% کف خالی
                else if (rand_val < 75)
                    map[y][x] = PILLAR; // 15% ستون
                else if (rand_val < 90)
                    map[y][x] = CORRIDOR; // 15% راهرو
                else
                    map[y][x] = DOOR; // 10% در
            }
        }
    ...
    // قرار دادن بازیکن در موقعیت تصادفی روی یک کف خالی
    /*int px, py;
    do
    {
        px = rand() % (WIDTH - 2) + 1;
...
void move_player(int *px, int *py, int direction)
{
    ...
    if (new_x >= 0 && new_x < WIDTH && new_y >= 0 && new_y < HEIGHT && map[new_y][new_x] != WALL)
    {
        map[*py][*px] = FLOOR;
        *px = new_x;
        *py = new_y;
...
{
    generate_map();
    int px = 3, py = 3; // موقعیت اولیه کاراکتر
    initscr();
    keypad(stdscr, TRUE);
    noecho();
    curs_set(0);
